const dataCtrl = () => {
    ...
    return {
        ...
        createShift: (month) => {
            ...
            shiftSchedule = [];
            shiftPool = ['M', 'N', 'O', 'O', 'L'];
            shiftSchedule.push(shiftPool[Math.floor(Math.random() * shiftPool.length)]);
        ...
        makeLeaveSchedules: (level, length) => {
            ...
            while (leaveSchedules.length < length) {
                leaveSchedules.push(dataCtrl().getLeaveSchedule(dataCtrl().getData().personObj(level), dataCtrl().setLeaveMonth()));
            }
...
/* console.log(dataCtrl().dataChunk());
console.log(dataCtrl().setLeaveMonth());
const person = dataCtrl().getData().personObj(7);
console.log(person);
const month = dataCtrl().getData().monthObj();
console.log(month.name());
console.log(month.length());
const shift = dataCtrl().createShift(month);
console.log(shift);
const shiftSchedule = dataCtrl().getLeaveSchedule(person, dataCtrl().setLeaveMonth());
console.log(shiftSchedule); */

/* 
    shifts:
        no of shifts to generate
        return an array of such shift
        let shifts = []
        while(shifts.length < no) {
            shifts.push(dataCtrl.createShift(month));
        }
